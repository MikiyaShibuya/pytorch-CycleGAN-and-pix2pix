...
class TransModalModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_A_GAN', 'G_A_L1', 'D_AB_real', 'D_AB_fake']
        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        self.visual_names = ['real_A_viz', 'fake_B', 'real_B']
        # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>
        if self.isTrain:
            self.model_names = ['G_A', 'D_AB']
    ...
    def backward_D(self):
        ...
        pred_real = self.netD_AB(real_AB)
        self.loss_D_AB_real = self.criterionGAN(pred_real, True)
    ...
    def backward_G(self):
        ...
        self.loss_G_A_GAN = self.criterionGAN(pred_fake_AB, True)
        # Second, G(A) = B
        self.loss_G_A_L1 = self.criterionL1(self.fake_B, self.real_B) * self.opt.lambda_L1
        # combine loss and calculate gradients
        self.loss_G = self.loss_G_A_GAN + self.loss_G_A_L1
